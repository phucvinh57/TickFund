        ...
        <ModalBody>
            <Container>
                ...
                <Row className='mb-2'>
                    ...
                    <Col className="me-2" sm={8} >{!data.category ? null : data.category.val.split().map(el => el.charAt(0).toUpperCase() + el.slice(1)).join(' ')}</Col>
                </Row>
                <Row className='mb-2'>
                    <Col className="ms-2">
                        <strong>Người giao dịch</strong>
                    </Col>
                    <Col className="me-2" sm={8} >{!data.user ? null : data.user.val}</Col>
                </Row>
                <Row className='mb-2'>
                    <Col className="ms-2">
...
export default function HistoryTable() {
    ...
    const filter = (str) => {
        const filteredData = multiFilter(mockDB, str)
